...
contract Vote {                   // Define the Vote contract
    ...
    struct Candidate {            // Structure for candidate details
    ...
    struct Voter {                // Structure for tracking voter’s status
    ...
    function changeName(string memory newName) external {
        ...
        require(candidates[idx].walletAddress == msg.sender,   // Verify candidate exists
            "Candidate not found");
    ...
    function getTopCandidate() external view returns (Candidate memory) {
        ...
        for (uint256 i = 1; i < candidates.length; i++) {            // Iterate through rest of candidates
            if (candidates[i].voteCount > topCandidate.voteCount) {  // Compare votes
